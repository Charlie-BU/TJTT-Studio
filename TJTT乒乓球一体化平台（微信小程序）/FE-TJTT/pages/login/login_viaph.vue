			...
			<view v-if="!hide_this" class="login">
				<view class="kuang shadow">
					...
					<view class="form-group flex ">
						...
						<input placeholder="请输入手机号" maxlength="100" class="ipt tpyA" type="tel"
							v-model="phone" placeholder-class="phc ipt" />
					</view>
					<view class="form-group flex">
					...
					<radio-group @change="whether_agree">
						<label class="radio" style="white-space: nowrap; display: flex;">
							<radio value="agreed" color="#FFCC33" style="transform:scale(0.7);" />同意小程序的
							<view @click="policy" style="color: blue;">协议与隐私政策</view>
						</label>
					</radio-group>
					<button class="btn btn1 round" @click="login">登录</button>
					<button class="btn btn2 round" style="background-color: #0bd618;" @click="wx_login">微信一键登录</button>
					<button class="btn btn2 round" @click="register_page">注册</button>
					<button class="btn btn2 round" @click="login_page">姓名登录</button>
					<button class="btn btn2 round" @click="forget_psw_page">忘记密码</button>
				</view>
			</view>
			<view class="bottom">
			</view>
	...
	export default {
		...
		methods: {
			...
			login() {
				...
				fetch_data("POST", "login_viaph", data, "user", res => {
